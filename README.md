...
![Groq](https://img.shields.io/badge/LLM-Llama%203.3%2070B%20%7C%20Groq-orange)
...
flowchart TD
    ...
    subgraph PIPELINE ["⚙️  LangGraph 5-Node Pipeline"]
        direction TB
...
|---|---|---|
| **Header** | ✅ | Lead ID, date, channel, priority (🔴🟡🟢) |
| **👤 Buyer Snapshot** | ✅ | Styled table — name, email, phone, budget, timeline, urgency |
| **🏠 What They're Looking For** | ✅ | 2–3 sentence LLM narrative |
| **⚠️ Realtor Alerts** | ✅ | Flags or *"None — clean lead"* |
| **🏆 Top Property Matches** | ❌ Vague leads | Scored properties (X/10), strengths, watch-outs, negotiation notes |
| **📋 Clarifying Questions** | ❌ Vague only | Discovery call questions for the realtor |
| **🤝 Negotiation Context** | ❌ Negotiation leads | CMA advisory paragraph |
| **💡 Suggested Next Action** | ✅ | Concrete, personalised realtor next step |
...
```bash
# Windows (required for emoji support)
...
│   ├── llm_utils.py                 # Shared LLM client — retry + TPM/TPD handling
│   ├── pdf_utils.py                 # Markdown → HTML → styled PDF converter
│   ├── nodes/
...
- **TPM limits** → handled automatically with exponential backoff (6 retries, 8s–65s)
- **Daily quota (TPD)** → agent prints a clear error; wait until **midnight UTC** to reset

> 💡 **Tip**: Run `python -X utf8 main.py` on a fresh daily quota. With 3-second inter-lead pauses, all 12 leads complete in **5–8 minutes**.
...
*Built for the AgentMira Engineering Case Study · Model: `llama-3.3-70b-versatile` via Groq*
